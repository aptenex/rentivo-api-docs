...
The Rentivo Lycan API implements a JWT based authentication. Any requests which require authentication will need to be   
...
To get Acquiring an access token is a three-step process:
...
```shell
$ http --form POST "https://lycan.rentivo.com/api/auth/token" \
...
```json
{
    "token": "ey23V9BRE1JTl9MWUNBTl9QUk9WSURFUl9IRkLLMsx0OIiwiUk9MRV9B"
}
```
